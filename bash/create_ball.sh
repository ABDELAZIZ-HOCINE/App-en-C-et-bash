...
if [ $# -lt 1 ]; then
    echo "Aucun argument passe au script."
    exit 1
fi
...
# Fonction pour traiter recursivement les fichiers et les dossiers
traiter_fichier() {
...
cat << TAGDEFIN > "$fichier".uu                     # creer un fichier identique au fichiers précédents
`cat "$fichier".uu`
TAGDEFIN
...
# traiter tous les argument passes au script
for arg in "$@";do
    traiter_fichier "$arg"
done
chmod u+x "$s"
